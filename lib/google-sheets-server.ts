...
// Функция для получения данных об успеваемости класса
export async function getClassPerformanceData() {
  try {
    const auth = await initGoogleAuth()
    const sheets = google.sheets({ version: "v4", auth })

    // Получаем данные о текущих оценках
    const currentGradesResponse = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${CURRENT_GRADES_SHEET}!${CURRENT_GRADES_RANGE}`,
    })

    // Получаем данные о прошлых оценках
    const previousGradesResponse = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${PREVIOUS_GRADES_SHEET}!${PREVIOUS_GRADES_RANGE}`,
    })
    ...
    // Структура для хранения средних баллов по четвертям
    const quarterAverages: Record<string, number> = {}

    // Структура для хранения средних баллов учеников по четвертям
    const studentQuarterAverages: Record<string, Record<string, number>> = {}

    // Структура для хранения общих средних баллов учеников
    const studentAverages: Record<string, { sum: number; count: number }> = {}

    // Обрабатываем оценки за прошлые четверти
    previousGradesValues.forEach((row) => {
      if (row.length < 2) return // Пропускаем неполные строки
      ...
      // Инициализируем структуры для ученика, если они еще не созданы
      if (!studentQuarterAverages[studentId]) {
        studentQuarterAverages[studentId] = {}
      }

      if (!studentAverages[studentId]) {
        studentAverages[studentId] = { sum: 0, count: 0 }
      }

      // Обрабатываем оценки по предметам
      subjects.forEach((subject, subjectIndex) => {
        const columnIndex = subjectIndex + 3 // D - это индекс 3
        ...
        // Обрабатываем оценки по четвертям
        gradesArray.forEach((gradeStr, qIndex) => {
          if (qIndex >= 4) return // Максимум 4 четверти

          const quarterIds = ["2025-Q1", "2025-Q2", "2025-Q3", "2025-Q4"]
          const quarterId = quarterIds[qIndex]

          const value = Number.parseInt(gradeStr, 10)

          if (!isNaN(value) && value > 0) {
            // Добавляем оценку к среднему баллу ученика за четверть
            if (!studentQuarterAverages[studentId][quarterId]) {
              studentQuarterAverages[studentId][quarterId] = { sum: 0, count: 0 }
            }

            studentQuarterAverages[studentId][quarterId].sum += value
            studentQuarterAverages[studentId][quarterId].count++

            // Добавляем оценку к общему среднему баллу ученика
            studentAverages[studentId].sum += value
            studentAverages[studentId].count++

            // Добавляем оценку к среднему баллу класса за четверть
            if (!quarterAverages[quarterId]) {
              quarterAverages[quarterId] = { sum: 0, count: 0 }
            }

            quarterAverages[quarterId].sum += value
            quarterAverages[quarterId].count++
          }
        })
      })
    })
    ...
    // Обрабатываем текущие оценки
    currentGradesValues.forEach((row) => {
      if (row.length < 2) return // Пропускаем неполные строки
      ...
      subjects.forEach((subject, subjectIndex) => {
        ...
        // Добавляем все оценки к текущей четверти
        gradesArray.forEach((gradeStr) => {
          const value = Number.parseInt(gradeStr, 10)

          if (!isNaN(value) && value > 0) {
            // Добавляем оценку к среднему баллу ученика за текущую четверть
            if (!studentQuarterAverages[studentId]["current"]) {
              studentQuarterAverages[studentId]["current"] = { sum: 0, count: 0 }
            }

            studentQuarterAverages[studentId]["current"].sum += value
            studentQuarterAverages[studentId]["current"].count++

            // Добавляем оценку к общему среднему баллу ученика
            studentAverages[studentId].sum += value
            studentAverages[studentId].count++

            // Добавляем оценку к среднему баллу класса за текущую четверть
            if (!quarterAverages["current"]) {
              quarterAverages["current"] = { sum: 0, count: 0 }
            }

            quarterAverages["current"].sum += value
            quarterAverages["current"].count++
          }
        })
      })
    })

    // Вычисляем средние баллы по четвертям
    const finalQuarterAverages: Record<string, number> = {}
    Object.entries(quarterAverages).forEach(([quarterId, data]) => {
      if (data.count > 0) {
        finalQuarterAverages[quarterId] = Number((data.sum / data.count).toFixed(2))
      }
    })

    // Вычисляем средние баллы учеников по четвертям
    const finalStudentQuarterAverages: Record<string, Record<string, number>> = {}
    Object.entries(studentQuarterAverages).forEach(([studentId, quarters]) => {
      finalStudentQuarterAverages[studentId] = {}

      Object.entries(quarters).forEach(([quarterId, data]) => {
        if (data.count > 0) {
          finalStudentQuarterAverages[studentId][quarterId] = Number((data.sum / data.count).toFixed(2))
        }
      })
    })

    // Вычисляем общие средние баллы учеников
    const finalStudentAverages: Record<string, number> = {}
    Object.entries(studentAverages).forEach(([studentId, data]) => {
      if (data.count > 0) {
        finalStudentAverages[studentId] = Number((data.sum / data.count).toFixed(2))
      }
    })

    // Создаем рейтинг учеников
    const studentRankings = getAllStudents()
      .map((student) => ({
        id: student.id,
        name: student.name,
        average: finalStudentAverages[student.id] || 0,
      }))
      .filter((student) => student.average > 0)
      .sort((a, b) => b.average - a.average)

    // Создаем распределение учеников по баллам
    ...
    Object.values(finalStudentAverages).forEach((average) => {
      if (average >= 9) distribution["9-10"]++
      else if (average >= 8) distribution["8-9"]++
      else if (average >= 7) distribution["7-8"]++
      else if (average >= 6) distribution["6-7"]++
      else if (average >= 5) distribution["5-6"]++
      else if (average >= 4) distribution["4-5"]++
      else distribution["0-4"]++
    })

    return {
      quarterAverages: finalQuarterAverages,
      studentQuarterAverages: finalStudentQuarterAverages,
      studentAverages: finalStudentAverages,
      studentRankings,
      distribution,
    }
  } catch (error) {
